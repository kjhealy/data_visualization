...
```{r 04-show-the-right-numbers-11, echo = FALSE, message = FALSE}
...
.pull-right[
```{r 04-show-the-right-numbers-12 }
df
```
]
...
`r chunk_reveal("reveal-pivlongex", widths = c(35,65), title = "# Get the data into long format!")`

```{r reveal-pivlongex, include = FALSE}
 df |> 
...
.pull-right.w50[
```{r 04-show-the-right-numbers-13, echo=FALSE}
  knitr::include_graphics(
...
---
`r chunk_reveal("reveal-pivottrick1", widths = c(45,55), title = "# A little pivot trick")`

```{r reveal-pivottrick1, include = FALSE}
...
---
`r chunk_reveal("reveal-pivottrick2", widths = c(45,55), title = "# A little pivot trick")`

```{r reveal-pivottrick2, include = FALSE}
...
.pull-left.w55[
```{r 04-show-the-right-numbers-14 }
p_out <- df |>
  pivot_longer(cols = pop_a:pop_c) |>
  ggplot() + 
...
- Something we haven't seen before, but will be using a lot: We can layer geoms one on top of the other. Here we call .kjh-green[`geom_histogram()`] twice. What happens if you comment one or other of them out?
...
```{r 04-show-the-right-numbers-15, echo = FALSE, fig.width=15, fig.height=5}
p_out
```
...
- By default .kjh-green[`geom_bar()`] tries to count up data by category. (Really, its the .kjh-green[`stat_count()`] function that does this behind the scenes.) By saying `stat="identity"` we explicitly tell it not to do that. This also allows us to use a `y` mapping. Normally this would be the result of the counting up.
