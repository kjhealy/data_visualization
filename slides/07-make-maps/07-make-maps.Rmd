...
---
`r chunk_reveal("reveal-countypop", widths = c(35,65), title = "# County Population Density")`
...
---
`r chunk_reveal("reveal-choroblack", widths = c(35,65), title = "# Same again for Percent Black")`
...
```{r, echo = FALSE, fig.height=0.5, fig.width=5}
par(mar = c(0, 0, 0, 0))
...
---
`r chunk_reveal("reveal-maxstates", widths = c(35,65), title = "# The two leading states in each region in 2014")`
...
---
`r chunk_reveal("reveal-opiateline", widths = c(35,65), title = "# Opiates Time Series plot")`
...
- It's very useful to have the intuition that, when drawing maps, we're just working with tables of `x` and `y` coordinates and shapes that represent quantities in our data, in a way that's very similar to any other geom. This makes it worth getting comfortable with what  .kjh-green[`geom_polygon()`] and .kjh-green[`coord_map()`] are doing. But the business of having very large map tables and manually specifying projections is inefficient. 

--

- In addition, sometimes our data _really is_ properly spatial, at which point we need a more rigorous and consistent way of specifying those elements. There's a whole world of Geodesic standards and tools devoted to specifying these things for GIS applications. R is not a dedicated GIS, but we can take advantage of these tools.

--

- Enter .kjh-pink[simple features], the .kjh-lblue[`sf`] package, and .kjh-green[`geom_sf()`]
